...
const Navbar = ({ logo }: { logo?: React.ReactNode }) => {
    ...
    const handleSignout = async () => {
        ...
        if (res.success) {
            ...
            console.log("Error in signing out : ", res.message);
        }
        setTimeout(async () => {
    ...
    //* After login
    useEffect(() => {
        const isFCMTokenFlag =
            sessionStorage.getItem("login-toast") ||
            sessionStorage.getItem("registered-toast"); //? Ask for notification permession and register fcm token
        if (isFCMTokenFlag) {
            registerFcmToken();
    ...
    useEffect(() => {
        if (session.data?.isRefreshTokenExpired) handleSignout();
    }, [session.data?.isRefreshTokenExpired]);
    return (
        <nav className="md:h-16 py-2 md:px-2 bg-background md:flex md:items-center border-b border-athens-gray flex-1 z-10">
            <div className="w-full px-2 mx-auto flex flex-col md:flex-row justify-between items-center gap-2">
                {logo ? (
                    <Link href={"/"} className="w-32 flex gap-2 items-center">
                        {logo}
