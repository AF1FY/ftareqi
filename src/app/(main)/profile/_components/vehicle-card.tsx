...
import type { CarDetails } from "@/types/Driver";
...
export default function VehicleCard({
    ...
    isDark: boolean;
    car: CarDetails | null | undefined;
    isLoading: boolean;
}) {
    if (isLoading) {
        return (
            <div
                className={`rounded-2xl border p-8 shadow-sm ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-200"}`}
            >
    ...
    const vehicleImage = car.carPhoto || "/placeholder.svg";
    const modelLabel = car.model || "Unknown model";
    const colorLabel = car.color || "Unknown color";
    const plateLabel = car.plate || "N/A";
    const expiryLabel = car.licenseExpiryDate
        ? new Date(car.licenseExpiryDate).toLocaleDateString("en-US", {
              day: "numeric",
              month: "short",
              year: "numeric",
          })
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row gap-8">
                {/* Car Image */}
                <div
                    className={`w-full md:w-5/12 aspect-4/3 rounded-xl overflow-hidden relative group border transition-colors ${
                        isDark
                ...
                >
                    ...
                    />
                    <div
                        className={`absolute bottom-3 left-3 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg uppercase tracking-wider`}
                    >
                        Primary
                    </div>
                </div>
                ...
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-6 content-start">
                    ...
                    <div className="group">
                        <label
                            className={`text-xs font-bold uppercase tracking-wider block mb-1 transition-colors ${
                                isDark
                    ...
                    <div className="group">
                        ...
                        <input
                            ...
                            type="number"
                            value={car.numOfSeats?.toString() || "N/A"}
                            readOnly
                        />
                    </div>
                    ...
                    <div className="group">
                        ...
                        <div className="flex items-center">
                            ...
                            <AlertCircle
                                size={18}
                                className={`ml-2 ${isDark ? "text-slate-600" : "text-gray-400"}`}
                            />
                        </div>
                    </div>
                    ...
                    <div className="sm:col-span-2 pt-2">
                        ...
                        >
                            <div>
                                <p
                                    className={`text-xs font-bold uppercase tracking-wider ${isDark ? "text-blue-400" : "text-blue-600"}`}
                                >
                                    Insurance Expiry
                                </p>
                                <p
                                    className={`text-sm font-bold mt-1 ${isDark ? "text-gray-200" : "text-gray-900"}`}
                                >
                                    {expiryLabel}
                                </p>
                            </div>
                            <Calendar
                                size={20}
                                className={`${isDark ? "text-slate-500 group-hover:text-blue-400" : "text-gray-400 group-hover:text-blue-600"}`}
                            />
                        </div>
                    </div>
                </div>
