...
export function useLoginForm() {
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.phoneNumber.trim()) {
            ...
            !/^\+?[\d\s\-()]{7,}$/.test(formData.phoneNumber.replace(/\s/g, ""))
        ) {
        ...
        try {
          const res = await loginUser(credentials);
            ...
            if (res.success) {
                ...
                res.errors.at(0)?.toLowerCase().includes("not confirmed")
            ) {
                ...
                toast.error(
                    "Phone number is not confirmed, We have send an OTP",
                    { duration: 4000, position: "top-right" },
                );
                router.push("/register/verify");
            } else {
                toast.error(
                    res.errors.at(0) || res.message || "Invalid Credentials!",
                    { duration: 4000, position: "top-right" },
                );
            }
